...
- **日期**：2026-05-15
- **状态**：Design 完成 · Plan 已写（见 `mianshigou-replica-plan.md`）
...
| 范围 | 深度 |
|---|---|
| 公开 Apple API 实现的能力（屏幕共享豁免、进程隐身用户态、录屏检测、显示器分离等） | 写出 **Swift 代码片段 + API 调用** |
| ASR / LLM / Electron / VAD 等基础设施 | 写到**可指导实施**的深度 |
| 对抗具体某款监考软件（Proctorio / 鹰眼 / 网易监考宝 等）的绕过 | **不写**。只写技术**类别 + 原理**。 |
...
§ 6.5 讨论的"对抗具体监考软件 / 对抗外部录屏 / 对抗行为指纹检测"**不作为模块落地**，仅作为技术类别分析。
...
设计完成后已进入实施计划阶段，见 `mianshigou-replica-plan.md`（M0-M10 共 50 个 TDD 任务）。
...
### v1 范围 → 完整能力 的差距（v2+ 增强项）

下面是 spec § 2 锁定范围里包含、但 v1 MVP plan 未实现、需要 v2 补的项：

- **屏幕识别 · 知名站点特化**：§ 2 锁定为"手动截图 + 持续自动检测 + LeetCode / 牛客等知名站点特化"，v1 plan 只实现前两者。**针对 LeetCode / 牛客 / HackerRank 的 DOM 注入或 URL 识别 + 题面结构化解析** 是 v2 项
- **QuestionClassifier · LLM 兜底**：§ 5.3 设计为"规则 + 小模型混合（未命中走 LLM 兜底）"，v1 plan 只实现规则匹配（未命中走 `general` 默认）。**接入小模型在 ambiguous 时做最终分类** 是 v2 项
